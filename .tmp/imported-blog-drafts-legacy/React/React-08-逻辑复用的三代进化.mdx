...
optimized_from: 016. Render Props.mdx + 017. Class & Hooks.mdx
...
function Dashboard() {
  ...
  return <div>{mouse.x} / {size.width}</div>;
}
```
...
function withMousePosition(WrappedComponent) {
  ...
  Wrapper.displayName =
    `WithMouse(${WrappedComponent.displayName || WrappedComponent.name})`;
  return Wrapper;
}
```
